...
class notEqual private(provider: Any => Any, _message: Option[String]) extends ValidationAnnotation:
  ...
  override protected def applyRule[P, T](validator: PropertyValidator[P, T]): PropertyValidationRule[P, T] =
    validator.mustNotBe.equal(obj => provider.apply(obj).asInstanceOf[P])
