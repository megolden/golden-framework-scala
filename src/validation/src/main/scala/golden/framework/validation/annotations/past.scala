...
class past private(utc: Boolean, _message: Option[String]) extends ValidationAnnotation:
  ...
  override protected def applyRule[P, T](validator: PropertyValidator[P, T]): PropertyValidationRule[P, T] =
    validator.mustBe.lessThan((value, _) => nowByTypeOf(value, utc).asInstanceOf[P])
